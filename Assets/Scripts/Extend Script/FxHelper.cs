...
public class FxHelper : MonoBehaviour {
	...
	// setiap awake
	public ParticleSystem swordClashParticle;
	void Awake()
	{
	...
	public void SwordFX(Vector3 position)
	{
		create(swordClashParticle, position);
	}

	//  crash fx
	public void CrashFX(Vector3 position)
	{
		create(crashParticle, position);
		create(deadParticle, position);
	}
	// deadfx
	public void DeadFX(Vector3 position)
	{
		create(deadParticle, position);
	}
